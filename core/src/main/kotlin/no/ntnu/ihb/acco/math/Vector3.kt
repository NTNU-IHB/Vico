...
data class Vector3(
    ...
    operator fun set(index: Int, value: Double): Vector3 {
        when (index) {
            0 -> x = value
            1 -> y = value
            2 -> z = value
    ...
    fun applyMatrix4(m: Matrix4): Vector3 {
        ...
        val w = 1.0 / (e[3] * x + e[7] * y + e[11] * z + e[15])
    ...
    fun setFromCylindricalCoords(radius: Double, theta: Double, y: Double): Vector3 {
    ...
    fun normalize(): Vector3 {
        var length = length()
        if (length.isNaN()) length = 1.toDouble()
