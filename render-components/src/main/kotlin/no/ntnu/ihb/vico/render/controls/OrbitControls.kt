package no.ntnu.ihb.vico.render.controls

/*
import no.ntnu.ihb.vico.core.EventDispatcher
import no.ntnu.ihb.vico.core.EventDispatcherImpl
import no.ntnu.ihb.vico.input.MouseEvent
import no.ntnu.ihb.vico.math.*
...
class OrbitControls(
    ...
    var minZoom = 0.0
    var maxZoom = Double.POSITIVE_INFINITY

    // How far you can orbit vertically, upper and lower limits.
    // Range is 0 to Math.PI radians.
    var minPolarAngle = 0.0 // radians
    var maxPolarAngle = PI // radians

    // How far you can orbit horizontally, upper and lower limits.
    // If set, must be a sub-interval of the interval [ - Math.PI, Math.PI ].
    var minAzimuthAngle = Double.NEGATIVE_INFINITY // radians
    var maxAzimuthAngle = Double.POSITIVE_INFINITY // radians
    ...
    var keyPanSpeed = 7   // pixels moved per arrow key push
    ...
    // for reset
    private var target0 = target.copy()
    ...
    private val panOffset = Vector3d()
    private var zoomChanged = false

    private val rotateStart = Vector2d()
    private val rotateEnd = Vector2d()
    private val rotateDelta = Vector2d()

    private val panStart = Vector2d()
    private val panEnd = Vector2d()
    private val panDelta = Vector2d()

    private val dollyStart = Vector2d()
    private val dollyEnd = Vector2d()
    private val dollyDelta = Vector2d()
    ...
    fun update(): Boolean {
        ...
        val quat = Quaterniond().setFromUnitVectors(camera.up, Vector3d_Y)
        val quatInverse = quat.copy().invert()

        val lastPosition = Vector3d()
        val lastQuaternion = Quaterniond()
        ...
        if (this.enableDamping) {
            ...
            panOffset.mul(1 - this.dampingFactor)

        } else {

            sphericalDelta.set(0.0, 0.0, 0.0)

            panOffset.set(0.0, 0.0, 0.0)
        ...
        if (zoomChanged ||
            lastPosition.distanceSquared(this.camera.position) > EPS ||
    ...
    fun getAutoRotationAngle(): Double {

        return 2 * PI / 60.0 / 60.0 * this.autoRotateSpeed

    }

    fun getZoomScale(): Double {

        return 0.95.pow(this.zoomSpeed)

    }

    fun rotateLeft(angle: Double) {

        sphericalDelta.theta -= angle

    }

    fun rotateUp(angle: Double) {
   ...
   */
/* fun panLeft(distance: Double, objectMatrix: Matrix4dc) {
    ...
    fun panUp(distance: Double, objectMatrix: Matrix4dc) {
